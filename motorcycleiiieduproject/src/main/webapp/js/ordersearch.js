...
$("#outtableDIV").html(
    $(`<table class='table-responsive table-bordered table-striped' id='dataTable' width='100%' cellspacing='0' style='table-layout: fixed; font-size: xx-small;'>
                ...
                for (let k in response) {
                    ...
                    if( response[k].orderStatus == "未來訂單"){
                        orderoption =  "<optgroup label='訂單狀態'>"+ "<option value='進行中訂單'>取車完成</option>" +
                        "<option value='已取消訂單'>取消訂單</option>" +"</optgroup>"
                        ...
                        orderoption = "<optgroup label='訂單狀態'>"+"<option value='已完成訂單'>環車完成</option>" +"</optgroup>"
                        orderoptionbutton =    "<td><button type='button' class='btn btn-success btn-sm finOrder'>完成訂單</button></td>"
                    }else if (response[k].orderStatus == "未來調度"){ 
                        orderoption =  "<optgroup label='調度狀態'>" +  "<option value='進行中調度'>進行調度</option>" +
                        "<option value='已取消調度'>取消調度</option>" +"</optgroup>"
                        ...
                        orderoption =  "<optgroup label='調度狀態'>" + "<option value='已完成調度'>完成調度</option>" +"</optgroup>"
                    ...
                    $("#mamnagerorderdetail").append(
                        ...
                        "<td>" +
                        "<select class='form-control form-control-sm' id ='selectresault'>" +
                        orderoption+
                        "</select>" +
                        "</td>" +
                        orderoptionbutton +
    ...
    $("#outtableDIV").on("click",  ".checkOrder" ,  function () {
